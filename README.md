...
|---|---|
| 1 | **사용자 입력** |
| 2 | **Custom Webhook**을 통한 요청 데이터 수신 |
| 3 | **Make**에서 입력 데이터 정리 및 플랫폼 분기 |
| 4-1 | **Instagram** → 텍스트 콘텐츠 생성 → 이미지 생성 프롬프트 → 이미지 생성 |
| 4-2 | **Blog** → 텍스트 콘텐츠 생성 → 이미지 생성 프롬프트 → 이미지 생성 |
| 4-3 | **X** → 텍스트 콘텐츠 생성 → 이미지 생성 프롬프트 → 이미지 생성 |
| 5 | **Google Drive**에 생성 이미지 저장 |
| 6 | **Notion 데이터베이스**에 생성 결과 기록 |

### 전체 처리 과정

1. 사용자가 주제, 키워드, 플랫폼, 톤 등의 정보를 입력합니다.
2. **Custom Webhook**이 요청 데이터를 수신합니다.
3. **Make**에서 입력 데이터를 정리하고 선택된 플랫폼에 따라 생성 흐름을 분기합니다.
4. **OpenAI**를 활용해 플랫폼별 텍스트 콘텐츠를 생성합니다.
5. 생성된 콘텐츠와 핵심 키워드를 바탕으로 이미지 생성 프롬프트를 구성합니다.
6. **OpenAI**를 활용해 플랫폼별 대표 이미지를 생성합니다.
7. 생성된 이미지를 **Google Drive**에 저장합니다.
8. 텍스트 결과, 이미지 링크, 요청 정보 등을 **Notion 데이터베이스**에 기록합니다.
...
| 팀원      | 담당 업무                                                                    |
| ------- | ------------------------------------------------------------------------ |
| **박경부** | 프로젝트 초기 기획 및 총괄, 자동화 시스템 설계·구현, 플랫폼 연동 테스트                           |
| **조근화** | 플랫폼별 프롬프트 설계·수정·보완, Notion 문서 구조·내용 정리·완성 |
| **박채원** | 자동화 구현 관련 학습                                                             |
| **김재남** | 프로젝트 보고서 초안 작성 참여 후 개인 사정으로 참여 중단                                     |
