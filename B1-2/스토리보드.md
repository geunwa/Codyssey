...
* **핵심 KPI(목표 지표)**: 영상 조회 대비 인스타그램 프로필 유입률 및 DM 상담 문의 전환 건수  
* **영상 목적 한 줄 요약**: 소상공인의 마케팅 막막함을 9초 안에 해소하고, 즉시 상담 문의로 연결시키는 브랜드 인지 확산 영상
...
* **제작 방식**: 실제 촬영(직촬) 소스 및 유료 스톡 이미지·영상을 전혀 사용하지 않고, 기획부터 최종 마스터링까지 전 공정을 생성형 AI로만 구현하여 브랜드 비주얼에 대한 완전한 통제력과 일관성을 확보하는 것을 제작 전략의 핵심으로 설정하였다.
...
* **적용 목적**: 60초 풀버전 영상을 채널별 규격(15초 숏폼)에 맞게 재구성할 경우 적용하는 컷 편집 전략  
* **유지 씬 우선순위**: 상황 제시(Scene 1) → 핵심 문제(Scene 2~3 대표 컷) → 결과 및 메시지 전달(Scene 4 엔드카드)  
* **축약 기준**: 설명적 전환 컷과 반복되는 디테일 숏을 제거하고, 시청자가 가장 빠르게 메시지를 인지할 수 있는 장면을 우선 배치  
* **구조 유지**: '도입 → 핵심 장면 → 메시지 전달'의 3단 구조를 15초 내에 압축 배치하여 숏폼으로서의 완결성을 확보
